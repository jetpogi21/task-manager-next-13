...
function getTaskSQL(
  ...
  if (!simpleOnly || simpleOnly !== "true") {
    ...
    if (query.hasFile === "true" && !dontFilter) {
      const taskNoteWithFileSQL = getTaskNoteSQL(query, dontFilter, true);
      const taskNoteWithFileJoin = new clsJoin(
        taskNoteWithFileSQL.sql.sql(),
        "id",
        `\`TaskNote.taskID\``,
        taskNoteWithFileSQL.subqueryAlias,
        "INNER"
      );
      sql.joins = [taskNoteWithFileJoin];
    }
    ...
    if (status && !dontFilter) {
      if (status === "finished") {
        filters.push(`${table}.is_finished`);
      } else {
        filters.push(`NOT ${table}.is_finished`);
      }
    }

    //Generated by GetDateBetweenFilter - Date In Between Filter
    const dateFrom = query.dateFrom as string;
    const dateTo = query.dateTo as string;

    if (dateFrom && dateTo && !dontFilter) {
      filters.push(`date BETWEEN :dateFrom AND :dateTo`);
...
function getTaskNoteSQL(
  ...
  if (withFilesOnly) {
    filters.push(`${table}.file IS NOT NULL`);
  }
  ...
  return {
    ...
    fieldAliases,
    replacements,
    subqueryAlias: withFilesOnly ? "tempTaskNoteWithFiles" : "tempTaskNote",
    modelName,
    filtered,
  };
}
